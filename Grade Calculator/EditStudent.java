...
import java.util.Date;
  ...
  { 
    JLabel  lblName, lblEmail, lblUsername, lblAddress, lblCountry, lblState, lblPhone, lblPassword, lblCity, lblPin;
    JTextField textName, textEmail, textUname,  textCountry, textState,  textPhone, textCity, textPin;
    JButton btnUpdate, btnCancel;
	JPasswordField textPwd;
	JComboBox comboCountry;
	JTextArea textAddress;
	JScrollPane scrollPane;
	String sName, sEmail, sUname, sAddr, sCountry, sState, sPhone, sPwd, sCity, sPin, sId;
	ArrayList<String> countryList = new ArrayList<String>();
	// Database Connectivity
		Connection connect = null;
		Statement statement = null;
		ResultSet resultSet,resultCountry,resultEmail= null;
    ...
    EditStudent(String class_name, String stud_roll, int xValue1, int yValue1)
	{
		setLayout(null);
		...
		cname	= class_name; sroll	= stud_roll;
		...
		try {
			...
			resultSet = statement.executeQuery("select * from user_master where class_assigned='"+class_name+"' and rollno='"+stud_roll+"'");
			//resultSet = statement.executeQuery("select * from faculty_master where email='gyanendra89@gmail.com'");
			// processing returned data and printing into console
			if(resultSet.next()){
				sId 		= new String(resultSet.getString("stud_id"));
				...
				sPwd 		= new String(resultSet.getString("password"));
		...
		textPwd			= new JPasswordField(sPwd);
       ...
       // setting position of Labels, Buttons aand TextField
        ...
        lblPassword.setBounds(80, 180, 200, 30);
        ...
        textPwd.setBounds(250, 180, 200, 30);
    ...
    public void actionPerformed(ActionEvent e) 
    {
		String command = e.getActionCommand();
         if(e.getSource() == btnUpdate)
         {
			...
			s11 	= new String(textPwd.getPassword());
			...
			if(!s1.isEmpty() && !s2.isEmpty()&& s2!=null && !s8.isEmpty() && !s9.isEmpty() && !s10.isEmpty() && !s12.isEmpty()){
               ...
               {
				    statement = connect.createStatement();
					statement.executeUpdate("update user_master set username='"+s16+"',fullname='"+s1+"', email='"+s2+"', address='"+s6+"', password='"+s11+"', contact_no='"+s10+"', country='"+s7+"', state='"+s8+"', city='"+s9+"', pin='"+s12+"', modified_at='"+modDate+"' where stud_id='"+sId+"' and class_assigned='"+cname+"' and rollno='"+sroll+"'");
    ...
    {	
        ...
        //new EditProfile();
		new EditStudent(title,userEmail, xValue, yValue);
    }
}
